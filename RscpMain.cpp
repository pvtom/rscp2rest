            ...
            for (std::vector<RSCP_MQTT::rec_cache_t>::iterator it = RSCP_MQTT::RscpMqttReceiveCache.begin(); it != RSCP_MQTT::RscpMqttReceiveCache.end(); ++it) {
                if ((it->done == false) || (it->refresh_until > now)) {
                    if (!it->container && !it->tag) { //system call
                        ...
                        if (!strcmp(it->topic, "log_errors")) logMessageCache(cfg.logfile, false);
                        if (!strcmp(it->topic, "health")) logHealth(cfg.logfile);
                        if (!strcmp(it->topic, "force")) refreshCache(RSCP_MQTT::RscpMqttCache, it->payload);
                        if ((!strcmp(it->topic, "interval")) && (atoi(it->payload) >= DEFAULT_INTERVAL_MIN) && (atoi(it->payload) <= DEFAULT_INTERVAL_MAX)) cfg.interval = atoi(it->payload);
                        if ((!strcmp(it->topic, "power_mode")) && cfg.auto_refresh) handleSetPower(RSCP_MQTT::RscpMqttReceiveCache, TAG_EMS_REQ_SET_POWER, it->payload);
                        if (!strcmp(it->topic, "limit_charge_soc")) storeIntegerValue(RSCP_MQTT::RscpMqttCache, 0, 0, atoi(it->payload), IDX_LIMIT_CHARGE_SOC, true);
                        if (!strcmp(it->topic, "limit_discharge_soc")) storeIntegerValue(RSCP_MQTT::RscpMqttCache, 0, 0, atoi(it->payload), IDX_LIMIT_DISCHARGE_SOC, true);
...
int main(int argc, char *argv[]) {
    ...
    while (i < argc) {
        ...
        if (!strcmp(argv[i], "--help")) {
            printf("rscp2p [%s] - Bridge between an E3/DC home power station and a fifo\n\n", RSCP2P);
            printf("Usage: rscp2p [options]\n\n");
            printf("Options:\n  -c, --config <file>\n  -d, --daemon\t\tdaemon mode\n  -v, --verbose\t\tverbose mode\n  -1, --once\t\tone interval runtime\n\n");
            printf("For more details please visit https://github.com/pvtom/rscp2p\n");
            exit(EXIT_SUCCESS);
        }
        i++;
    }
...
// Issue #9 
    ...
    if (cfg.wallbox) {
        for (uint8_t c = 0; c < cfg.wb_number; c++) {
            char topic[TOPIC_SIZE];
            sprintf(topic, "wallbox_%d_sun_mode", c + 1);
